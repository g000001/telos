...
(defmacro eval-when ((&rest when) &body body)
  (let ((when (mapcar (lambda (x)
                        (cdr (assoc x '((compile . :compile-toplevel)
                                        (load . :load-toplevel)
                                        (eval . :execute)))))
                      when)))
    `(cl:eval-when (,@when)
...
(defconstant class-slot-defaults
  (list #'(lambda () :anonymous)
        unbound
        #'(lambda () (list (symbol-value '<object>)))
        #'(lambda () ())
        #'(lambda () ())
        #'(lambda () ())
        unbound
...
(defconstant gf-slot-defaults
  (list #'(lambda () :anonymous)
        :required
        #'(lambda () (list (symbol-value '<simple-method>)))
        #'(lambda () ())
        #'(lambda () ())
        unbound
        unbound
...
(defvar cl-class-table (make-hash-table :test #'eq))
...
(def-cl-class simple-string <string> (<vector>) (<vector> <sequence> <array>))
...
corresponding values removed"
  ...
  (defun proclaim-gf (name arglist)
    (let ((args (mapcar #'(lambda (a) (if (eq a '&rest) '&rest t))
                        (gf-args arglist) )))
      `(proclaim '(ftype (function ,args t) ,name)) ))

  ) ; end of eval-when

; allows (defgeneric (setf foo) ...)

(defmacro defgeneric (gfname arglist &body keywords)
...
(defmacro generic-lambda (arglist &body keywords)
...
; c-n-m
(defmacro method-function-lambda (args &body body)
...
(defmacro named-method-function-lambda (name args &body body)
  `#'(lambda (.method-list. .argument-list. ,@args) ,@(block-body name body)))

(defmacro method-lambda (&body form)
...
(defmacro defmethod (gfun &body form)
...
(defun do-reader (acc name slotname)
  `((defvar ,acc () ,(format () "The ~s-~s slot reader" name slotname))
    (proclaim '(ftype (function (t) t) ,acc))
...
(defun do-writer (setter name slotname)
  `((defvar ,setter () ,(format () "The ~s-~s slot writer" name slotname))
    (proclaim '(ftype (function (t t) t) ,setter))
...
(test telos
  ...
  (defclass <bar> (<foo>) (d))
  (defstruct .s a b c)
  (defgeneric .gf ((x <object>))
    :method (((x <object>)) x))
  (defmethod .gf ((x <symbol>)) :symbol)
  (defmethod .gf ((x <string>)) :string)
  (defmethod .gf ((x <list>)) :list)
  (defmethod .gf ((x <vector>)) :vector)
  (defmethod .gf ((x <structure>)) :structure)
  (defmethod .gf ((x <foo>)) :foo)
  (defmethod .gf ((x <bar>)) :bar)
  ;;
  (is (= 3 (.gf 3)))
  (is (eq :symbol (.gf 'a)))
;  (is (eq :string (.gf "string"))) ???
  (is (eq :list (.gf '(a b c))))
  (is (eq :vector (.gf #(a b c))))
;  (is (eq :structure (.gf (make-.s))))
  (is (eq :foo (.gf (make <foo>))))
  (is (eq :bar (.gf (make <bar>)))))

#|(mapcar (lambda (x)
          (generic-funcall
           (generic-lambda ((x <object>))
             :method (((x <object>)) (list x))
             :method (((x <number>)) (* 2 x)))
           x))
        '(1 2 3 4))|#
